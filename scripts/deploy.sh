...
if [ "$#" -ne 6 ]; then
    echo "Usage: $0 <TEMPLATE_NAME> <TEMPLATE_DIR> <DEPLOYMENT_VARIANT> <SOURCE_JAR> <FRONTEND_ZIP> <DEPLOYMENT_INPUTS>"
    exit 1
fi
...
if [ ! -f "$FRONTEND_ZIP" ]; then
    echo "Error: Frontend ZIP file '$FRONTEND_ZIP' not found."
    exit 1
fi
...
cp "$FRONTEND_ZIP" "$DEST_DIR/frontend.zip"
echo "Copied $(basename "$FRONTEND_ZIP") to $DEST_DIR"
